...
function rtspUrl(camera: Camera): string {
...
export class StreamManager {
  async syncStreams(cameras: Camera[]): Promise<void> {
    ...
    await Promise.allSettled(
      online.map((cam) =>
        fetch(`${MEDIAMTX_API}/v3/config/paths/add/${encodeURIComponent(cam.camera_id)}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ source: rtspUrl(cam), sourceOnDemand: true }),
        }).then((r) => {
  ...
  async getStreamStatus(cameraId: string): Promise<{ active: boolean; readers: number }> {
    ...
    if (!res.ok) throw new Error(`MediaMTX GET path ${cameraId} → ${res.status}`);
