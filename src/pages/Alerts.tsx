...
export default function AlertsPage() {
  ...
  const handleResolve = (id: string) => {
    dataService.resolveAlert(id, 'Eduardo (Proprietário)');
  ...
  return (
    ...
    >
      ...
      <span className={cn(
        ...
        active ? "bg-primary text-white" : "bg-surface-alt text-text-dim"
      )}>
        {count}
      </span>
    </button>
...
const AlertCard: React.FC<AlertCardProps> = ({ alert, onResolve }) => {
  ...
  const handleWhatsAppShare = () => {
    window.open(notificationService.getWhatsAppLink(alert), '_blank');
  };
      ...
      <div className="p-6 md:p-8 flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-4">
          ...
          <p className="text-text-dim text-[13px] leading-relaxed font-medium">
            O motor de regras detectou este comportamento anômalo baseado no cruzamento de dados de {alert.type === 'card_gap' ? 'sistemas financeiros externos' : 'sensores de presença e vendas'}.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-2 text-xs">
             ...
             <div className="bg-surface-alt p-3 rounded border border-border">
                ...
                <div className="mt-1 font-mono font-bold text-text uppercase truncate">
                   {alert.type}
                </div>
             </div>
             {alert.context.diff && (
